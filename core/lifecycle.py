...
class DetectionModelTrainer(ModelTrainer):
    """Represents a model trainer, which trains a object detection model."""
    ...
    def train_for_one_epoch(self) -> float:
        ...
        # Cycles through the entire training subset of the dataset and trains the model on the samples (this is for one epoch of training)
        metric = torchmetrics.detection.mean_ap.MeanAveragePrecision().to(self.device)
        for inputs, bboxes in self.training_data_loader:
            self.model.train()
            ...
            # Performs a forward pass through the neural network
            with torch.autocast(device_type=self.device.type):
                loss = self.model(images=inputs, targets=bboxes)
            ...
            self.model.eval()
            with torch.no_grad():
                detections = self.model(images=inputs, targets=bboxes)
                acc = metric(detections, bboxes)
...
class DetectionModelValidator(ModelValidator):
    """Represents a model validator, which validates a reconstruction-based anomaly detection model on the validation subset of the dataset."""
    ...
    def validate(self) -> float:
        ...
        with torch.no_grad():
            ...
            # Cycles through the whole validation subset of the dataset and performs the validation
            metric = torchmetrics.detection.mean_ap.MeanAveragePrecision().to(self.device)
            for inputs, bboxes in self.validation_data_loader:
